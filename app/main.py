...
class Git:
    ...
    def cat_file(self, args): 
        ...
        flag = args[0]
        hash_str = args[1]
        object_path = os.path.join(self.git_dir, 'objects', hash_str[:2], hash_str[2:]) # the object associated with the given hash
    ...
    def hash_object(self, args):
        ...
        file_content_size = os.path.getsize(args[1])
        file_content_with_header = 'blob ' + str(file_content_size) + '\x00' + file_content
        sha1_of_file = self._compute_sha1_hash(file_content_with_header)
        compressed_data = zlib.compress(file_content_with_header.encode('utf-8'), level=9) 
        
        # priting the hash to stdout
        print(sha1_of_file)

        # making dir in object using the hash result
        path_new_object_dir = os.path.join(self.git_dir, 'objects', sha1_of_file[:2])
    ...
    # Computing sha1 hash 
    def _compute_sha1_hash(self, input_string): 
        # create object of hashlib -> update with utf encoded file data -> hexdigest function
